...
export default function ProductPage() {
  ...
  // WhatsApp Link Logic
  const productLink = product ? `https://laptopsolutions.shop/product/${product._id || product.id}` : "";
  const whatsappMessage = product ? `Hello, I want to ask about or buy this product: *${product.title}*.\nPrice: ₹${product.price}\nLink: ${productLink}` : "";
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 md:py-12 pb-24 md:pb-12"> {/* Added padding bottom for mobile bar */}
              ...
              <div className="mt-auto space-y-4 hidden md:block">
                <div className="flex gap-4">
                  ...
                  <a 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 whitespace-nowrap bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-4 px-6 rounded-xl transition-all shadow-lg shadow-yellow-500/20 text-lg"
                  >
                  ...
                  {/* Call Button */}
                   <a 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-200 px-6 py-4 rounded-xl font-bold text-lg hover:bg-indigo-100 transition-all"
                  >
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 md:hidden z-40 flex items-center gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <button 
            onClick={handleAddToWishlist}
            className="p-3 bg-gray-100 rounded-lg text-gray-500 hover:text-pink-500 hover:bg-pink-50 transition-colors"
        >
...
const ProductCard = ({ product }: { product: any }) => {
  ...
  // Removed Cart Icon Logic from here as requested
  ...
  return (
    ...
    >
        <button
            onClick={handleAddToWishlist}
            className="absolute top-2 right-2 md:top-3 md:right-3 z-10 bg-white/90 p-1.5 md:p-2 rounded-full text-gray-400 hover:text-pink-500 hover:bg-pink-50 shadow-sm opacity-100 md:opacity-0 group-hover:opacity-100 translate-y-0 duration-300 transition-colors"
        >
      ...
      <div className="flex flex-col flex-grow px-1">
        ...
        <div className="mt-auto flex flex-col gap-2">
          ...
          <a 
               ...
               rel="noopener noreferrer"
               onClick={(e) => e.stopPropagation()}
               className="flex items-center justify-center gap-2 whitespace-nowrap bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-2 px-4 rounded-lg transition-all shadow-md shadow-yellow-500/20 text-sm"
            >
